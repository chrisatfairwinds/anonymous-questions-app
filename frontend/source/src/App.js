...
const App = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await fetch(`${REACT_APP_SERVER_URL}/api/v0/questions`);
  ...
  const upVote = (question) => async () => {
    ...
    try {
      const response = await fetch(
        `${REACT_APP_SERVER_URL}/api/v0/questions/${question.ID}/up-vote`,
        {
          method: "POST",
          headers: {
  ...
  const deleteAllQuestions = async () => {
    try {
      await fetch(`${REACT_APP_SERVER_URL}/api/v0/questions`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
